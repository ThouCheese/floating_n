...
constexpr BitArray<N> BitArray<N>::operator<<=(int const n)
{
    ...
    // the the word at then index `s_length - skip`, so we start our get_loop
    // there. We then work left to right all the way to zero.

    // note: segfaults when `s_length - skip < 0`, so check that
    for (size_t index = s_length - skip; index-- and s_length > skip;)
    {
    ...
    for (size_t index = 0;
         index != n / WORD_LENGTH and index != s_length;
         ++index)
        d_data[index] = 0;
    ...
    WORD constexpr mask = 0xFFFFFFFFFFFFFFFF >> (s_length * WORD_LENGTH - N);
    d_data[s_length - 1] &= mask;
    return *this;
}
