...
constexpr BitArray<N> BitArray<N>::operator>>=(int const n)
{
    ...
    // s_length, so it is ill-advised to use `!=` instead of `<`.
    for (size_t index = skip; index < s_length; ++index)
    {
        ...
        // word to the left
        if (index != s_length - 1 and shift != 0)
    ...
    // now we need to to clear leftmost n / WORD_LENGTH WORDS
    for (size_t index = s_length;
         index-- != s_length - n / WORD_LENGTH and index !=  0;)
        d_data[index] = 0;
